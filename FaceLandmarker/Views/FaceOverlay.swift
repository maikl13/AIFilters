...
class OverlayView: UIView {
    private var dragonImage: UIImage? = UIImage(named: "dragon")
    private var glassesImage: UIImage? = UIImage(named: "glasses")
  ...
  override func draw(_ rect: CGRect) {
    for faceOverlay in faceOverlays {
     // drawDots(faceOverlay.dots)
      ...
      //  drawLines(lineConnection.lines, lineColor: lineConnection.color)
        ...
        addGlass(faceOverlay)
        addDragon(faceOverlay)
  
    }
  }

    func addGlass(_ faceOverlay : FaceOverlay ){
        
        if let glassesImage = glassesImage, faceOverlay.dots.count > 263 {
            let leftEye = faceOverlay.dots[263]
            let rightEye = faceOverlay.dots[33]
            let nose = faceOverlay.dots[168]

            let dx = leftEye.x - rightEye.x
            let dy = leftEye.y - rightEye.y
            let angle = atan2(dy, dx)
            let distance = hypot(dx, dy)
            let scale: CGFloat = (distance / glassesImage.size.width * 2.0) * 0.8
            ...
            // Yaw (يمين / يسار)
            let yawAmount = nose.x - centerX
            let yawFactor = CGFloat(1.0 / distance) * 4
            let yawAngle = yawAmount * yawFactor

            // Pitch (أعلى / أسفل)
            let pitchAmount = centerY - nose.y
            let pitchFactor = CGFloat(1.0 / distance) * 6
            let pitchAngle = pitchAmount * pitchFactor

            // Offset حسب اتجاه الرأس
            let xOffset = yawAngle * 40
            let yOffset = pitchAngle * 10 // جديد: يجعل النظارة تتحرك لأعلى/أسفل مع الرأس
            let adjustedCenter = CGPoint(x: center.x + xOffset, y: center.y - yOffset)
            ...
            if let context = UIGraphicsGetCurrentContext() {
                ...
                transform = CATransform3DTranslate(transform, adjustedCenter.x, adjustedCenter.y, 0)
                transform = CATransform3DRotate(transform, pitchAngle, 1, 0, 0)  // Pitch (أعلى / أسفل)
                transform = CATransform3DRotate(transform, yawAngle, 0, 1, 0)    // Yaw (يمين / يسار)
                transform = CATransform3DRotate(transform, angle, 0, 0, 1)       // Roll (ميل الرأس)
                transform = CATransform3DScale(transform, scale, scale, 1)
                transform = CATransform3DTranslate(transform, -glassesImage.size.width / 2, -glassesImage.size.height / 2, 0)

                context.concatenate(CATransform3DGetAffineTransform(transform))
                glassesImage.draw(at: .zero)
    ...
    func addDragon(_ faceOverlay : FaceOverlay ){
        
        if let glassesImage = dragonImage, faceOverlay.dots.count > 263 {
            let leftEye = faceOverlay.dots[263]
            let rightEye = faceOverlay.dots[33]
            let nose = faceOverlay.dots[168]
            ...
            // Offset حسب اتجاه الرأس
            let xOffset = yawAngle * 40
            let yOffset = pitchAngle * 35 // جديد: يجعل النظارة تتحرك لأعلى/أسفل مع الرأس
            let adjustedCenter = CGPoint(x: center.x + xOffset, y: center.y - yOffset)
            ...
            if let context = UIGraphicsGetCurrentContext() {
                ...
                var transform = CATransform3DIdentity
                transform.m34 = -1.0 / 500
                transform = CATransform3DTranslate(transform, adjustedCenter.x, adjustedCenter.y - 130, 0)
                transform = CATransform3DRotate(transform, pitchAngle, 1, 0, 0)  // Pitch (أعلى / أسفل)
                transform = CATransform3DRotate(transform, yawAngle, 0, 1, 0)    // Yaw (يمين / يسار)
                transform = CATransform3DRotate(transform, angle, 0, 0, 1)       // Roll (ميل الرأس)
                transform = CATransform3DScale(transform, scale, scale, 1)
